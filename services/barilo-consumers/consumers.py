...
from services import CircularProductService, CircularProductCreated
...
class BaseConsumer(ABC):
    ...
    def on_channel_open(self, channel):
        ...
        # I already have the exchange declared
    ...
    def on_message(self, _unused_channel, basic_deliver, properties, body):
        ...
        try:
            self.consume_message(basic_deliver.routing_key, body)
...
class CircularProductConsumer(BaseConsumer):
    svc: CircularProductService

    def __init__(self, amqp_url, svc: CircularProductService):
    ...
    def consume_message(self, topic: str, body: bytes):
        ...
        if topic == "circularproduct.created.key":
            try:
                params = CircularProductCreated(**message)
            except Exception as e:
                logger.info(
                    "👾 [x] Message does not match CircularProductCreated schema: %s",
                    e=e,
                )

            try:
                self.svc.update_circular_product_price_rank(params)
            except Exception as e:
                logger.info(
...
class CircularProductWorker(BaseWorker):
    def __init__(self, consumer: CircularProductConsumer):
