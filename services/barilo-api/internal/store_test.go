...
func TestProduct_FromCSV(t *testing.T) {
	...
	t.Run("valid csv row", func(t *testing.T) {
		...
		row := []string{"7891000055123", "test-name", "test-brand", "test-category", "test-weight", "2024-12-25", "23.99"}
		...
		row = []string{"", "test-name", "test-brand", "test-category", "test-weight", "2024-12-25", "23.99"}
		p = internal.Product{}
		err = p.FromCSV(ctx, row)
		assert.NoError(t, err)
	})
	...
	t.Run("invalid gtin", func(t *testing.T) {
		...
		row := []string{"invalid-gtin", "test-name", "test-brand", "test-category", "test-weight", "2024-12-25", "23.99"}
	...
	t.Run("invalid name", func(t *testing.T) {
		...
		row := []string{"invalid-gtin", "", "test-brand", "test-category", "test-weight", "2024-12-25", "23.99"}
