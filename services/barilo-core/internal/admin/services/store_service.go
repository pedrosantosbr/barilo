...
func (s *Store) Create(ctx context.Context, params internal.CreateStoreParams) (internal.Store, error) {
	if err := params.Validate(); err != nil {
		return internal.Store{}, errors.WrapErrorf(err, errors.ErrorCodeInvalidArgument, "params.Validate")
	}
...
func (s *Store) Find(ctx context.Context, params internal.FindStoreParams) (*internal.Store, error) {
	...
	if err != nil {
		return &internal.Store{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "repo.Find")
	}
...
func (s *Store) CreateCircularWithDiscounts(ctx context.Context, params internal.CreateCircularParams) (internal.Circular, error) {
	...
	if err := params.Validate(); err != nil {
		return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeInvalidArgument, "params.Validate")
	}

	// Find store
	store, err := s.Find(ctx, internal.FindStoreParams{ID: &params.StoreID})
	if err != nil {
		return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "s.Find")
	}
	if store == nil {
		return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeNotFound, "store not found")
	}
	...
	if err != nil {
		return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeInvalidArgument, "time.Parse")
	}
	circular.ExpirationDate = expirationDate
	if err := s.circularRepo.Create(ctx, &circular); err != nil {
		return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "repo.CreateCircular")
	}
	...
	for _, p := range params.Products {
		...
		if err != nil {
			return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "s.FindProduct")
		}
		if exists.ID != "" {
			p.ID = exists.ID
		...
		if err := s.productRepo.Create(ctx, p); err != nil {
			return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "s.CreateProduct")
		}
		circularProducts = append(circularProducts, *p)
	...
	for _, p := range circularProducts {
		...
		if err != nil {
			...
			return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "s.FindByProductID")
		}
		if exists.ID != "" {
		...
		if err := s.discountRepo.Create(ctx, &discount); err != nil {
			...
			return internal.Circular{}, errors.WrapErrorf(err, errors.ErrorCodeUnknown, "s.CreateDiscount")
		}
	}
