...
func renderErrorResponse(w http.ResponseWriter, r *http.Request, msg string, err error) {
	...
	var ierr *internalerrors.Error
	if !errors.As(err, &ierr) {
		resp.Error = "internal error"
	} else {
		switch ierr.Code() {
		case internalerrors.ErrorCodeNotFound:
			status = http.StatusNotFound
		case internalerrors.ErrorCodeInvalidArgument:
		...
		case internalerrors.ErrorCodeConflict:
			status = http.StatusConflict
		case internalerrors.ErrorCodeUnknown:
			fallthrough
		default:
			status = http.StatusInternalServerError
		}
